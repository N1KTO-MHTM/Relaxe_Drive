...
export default function LiveWall() {
  ...
  useEffect(() => {
    ...
    api
      ...
      >(
        `/reports?minLat=${minLat}&maxLat=${maxLat}&minLng=${minLng}&maxLng=${maxLng}&sinceMinutes=1`,
      )
  ...
  const filteredDrivers = useMemo(() => {
    ...
    const byId = list.filter(
      (d) => (d.id || '').toLowerCase().includes(q) || (d.driverId || '').toLowerCase().includes(q),
    );
  ...
  const activeCount = orders.filter(
    (o) => o.status === 'SCHEDULED' || o.status === 'ASSIGNED' || o.status === 'IN_PROGRESS',
  ).length;
  ...
  return (
    ...
    >
      ...
      >
        <h1 style={{ margin: 0, fontSize: '1.1rem' }}>{t('nav.liveWall')}</h1>
        <input
          type="text"
        ...
        <button
          type="button"
          className="rd-btn rd-btn-secondary"
          onClick={() => setReportsTrigger((r) => r + 1)}
        >
          {t('common.refresh')}
        </button>
        {driverRouteData && (
        ...
        <span className="rd-text-muted live-wall-status" style={{ fontSize: '0.875rem' }}>
          ...
          {Object.keys(typeCounts).length > 0 && (
            <span className="live-wall-types">
              {' '}
              (
