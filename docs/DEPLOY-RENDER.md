...
## Если деплой relaxdrive-api упал (deploy failed)

1. В Render открой сервис **relaxdrive-api** → вкладка **Logs**.
2. Прокрути до конца и найди красную ошибку (часто в разделе **Build** или **Deploy**).
3. Пришли сюда текст ошибки или скрин — подскажу, что поменять.
