...
- **drivers** — Drivers.tsx, Drivers.css. Таблица водителей (nickname, phone, email, driverId, carType, carPlateNumber, status). Вкладки по типу авто (All, Sedan, Minivan, SUV, Other). Поиск, экспорт CSV, кнопка «Show on map» (Link to /dashboard). При выборе водителя (клик по строке или «View details») открывается панель с тремя вкладками: Driver information (ник, телефон, email, Driver ID, тип авто, номер, модель/год), Trip history (карточки поездок: маршрут pickup → dropoff, время начала–конца, расстояние в милях, длительность, заработок за поездку), Earnings (всего заработано, всего миль; подпись про последние 7 дней). Данные: GET /users/:id/stats, GET /users/:id/trip-history.
