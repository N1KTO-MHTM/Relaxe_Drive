...
## Если Cursor падает с OOM (out of memory)

1. **При диалоге ошибки:** поставьте галочку **«Don't restore editors»** и нажмите Reopen.
2. **Лучший способ снизить OOM:** запускайте **Open-Cursor-Web.bat** или **Open-Cursor-Backend.bat** из корня проекта (откроется Cursor с одной папкой). Или вручную: File → Open Folder → папка `web` или `backend`. Индексации меньше — память не переполняется.
3. Подробнее: **OOM-ЧТО-ДЕЛАТЬ.txt** в корне, **CURSOR-TIPS.md**, **docs/КАК-УЛУЧШИТЬ-ПАМЯТЬ-CURSOR.md**.
