...
export default function WhiteLabelMode() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    api
      ...
      .then(() => setMessage(t('whiteLabel.saved')))
  ...
  return (
    <DesktopLayout>
      <div className="white-label-mode">
        ...
        {message && <p style={{ marginBottom: '1rem', color: 'var(--rd-success, #6a6)' }}>{message}</p>}
        {!loading && (
          <form onSubmit={handleSubmit} style={{ marginTop: '1rem', maxWidth: 400 }}>
            <label style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem', marginBottom: '1rem' }}>
              ...
              <input type="url" className="rd-input" value={logoUrl} onChange={(e) => setLogoUrl(e.target.value)} placeholder="https://…" />
            </label>
            <label style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem', marginBottom: '1rem' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                ...
                />
                <input type="text" className="rd-input" value={primaryColor} onChange={(e) => setPrimaryColor(e.target.value)} placeholder="#2563eb" style={{ flex: 1 }} />
              </div>
            </label>
            <label style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem', marginBottom: '1rem' }}>
              <span>{t('whiteLabel.domain')}</span>
              <input type="text" className="rd-input" value={domain} onChange={(e) => setDomain(e.target.value)} placeholder="app.example.com" />
            </label>
            <label style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem', marginBottom: '1rem' }}>
              <span>{t('whiteLabel.locales')}</span>
              <input type="text" className="rd-input" value={locales} onChange={(e) => setLocales(e.target.value)} placeholder="en,ru,ka,es" />
            </label>
            <button type="submit" className="rd-btn rd-btn-primary" disabled={saving}>
              {saving ? '…' : t('whiteLabel.save')}
            </button>
          </form>
        )}
      </div>
