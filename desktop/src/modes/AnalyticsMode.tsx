    ...
    <DesktopLayout>
      <div className="analytics-mode">
        ...
        <div style={{ display: 'flex', gap: '1rem', alignItems: 'center', flexWrap: 'wrap', marginBottom: '1rem' }}>
          <label>
            {t('analytics.from')}
            <input type="date" value={from} onChange={(e) => setFrom(e.target.value)} style={{ marginLeft: '0.5rem' }} className="rd-input" />
          </label>
          <label>
            {t('analytics.to')}
            <input type="date" value={to} onChange={(e) => setTo(e.target.value)} style={{ marginLeft: '0.5rem' }} className="rd-input" />
          </label>
        ...
        {!loading && !error && stats && (
          <>
            ...
            <p className="rd-text-muted" style={{ marginBottom: '0.75rem' }}>{t('analytics.heatmapHint')}</p>
            {stats.heatmap.length === 0 ? (
              <p className="rd-text-muted">{t('analytics.noData')}</p>
            ) : (
              (() => {
                ...
                return (
                  <div style={{ overflowX: 'auto' }}>
                    <table className="logs-mode__table" style={{ fontSize: '0.75rem', minWidth: 400 }}>
                      <thead>
                        <tr>
                          <th style={{ padding: '0.25rem 0.5rem', textAlign: 'left' }}></th>
                          {Array.from({ length: 24 }, (_, h) => (
                            <th key={h} style={{ padding: '0.25rem 2px', fontWeight: 500 }}>{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {dayNames.map((day, dayIdx) => (
                          <tr key={day}>
                            ...
                            {Array.from({ length: 24 }, (_, hour) => {
                              ...
                              const bg = intensity > 0 ? `rgba(79, 114, 158, ${0.2 + 0.7 * intensity})` : 'var(--rd-bg-elevated, #1e1e1e)';
                              return (
                                <td
                                  key={hour}
                                  style={{ padding: 2, background: bg, minWidth: 18, textAlign: 'center', borderRadius: 2 }}
